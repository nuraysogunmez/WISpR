...
def _assemble_spot(cnt : np.ndarray,
	...
	# sample between 10 to 30 cells to be present
    # at spot
    # n_cells = dists.uniform.Uniform(low = lower_bound,
                                    # high = upper_bound).sample().round().type(t.int)
     # sample between 10 to 30 cells to be present
    # at spot
    
    # n_cells = dists.uniform.Uniform(low = bounds[10],
                                  #   high = bounds[30]).sample().round().type(t.int)
...
def main():
    ...
    for k,v in assembled_set.items() :
        out_pth = osp.join(out_dir, 'trial1.'.join([k,args.tag,'tsv']))
        v.to_csv(out_pth,
                    sep = '\t',
                    index = True,
                    header = True,
